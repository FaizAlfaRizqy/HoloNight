...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirmPassword)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $hashed_password_esc = mysqli_real_escape_string($koneksi, $hashed_password);
            
            // Insert user
            $insert_query = "INSERT INTO users (username, email, password) 
                           VALUES ('$username_esc', '$email_esc', '$hashed_password_esc')";
            
            if (mysqli_query($koneksi, $insert_query)) {
                $success = 'Selamat anda telah menjadi Warga Nasional Hollownest';
